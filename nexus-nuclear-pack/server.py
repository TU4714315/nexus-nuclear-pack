"""
Nexus Intelligence Runtime — MCP Server  (v0.1.0)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Architecture (per NUCLEAR_SPEC.md):
  ...
  Policy Gate                →  allowlist / mode enforcement (defensive)
...
SPEC_VERSION = "0.1.0"
SERVER_NAME = "Nexus-Intelligence-Runtime"
...
# Risky patterns for config scanning (defensive – your own files only)
...
@dataclass
class Target:
    kind: str  # "repo" | "folder" | "logbundle" | "config"
    value: str  # local absolute path or owned repo URL
    proof: Optional[str] = None  # optional ownership proof token
...
class EvidenceLedger:
    """Hash-signed, append-only SQLite ledger for all runtime results."""
    ...
    def start_run(
        self, run_id: str, budget: Budget, policy_hash: str, goal: str
    ) -> None:
    ...
    def replay_run(self, run_id: str) -> dict:
        """Return full run record + all evidence for auditing / replay diff."""
        with self._connect() as conn:
            conn.row_factory = sqlite3.Row
            run = conn.execute(
...
class PolicyGate:
    """Enforces allowlist + mode. Defensive mode is immutable."""
    ...
    def _load(self, path: Path) -> dict:
        if path.exists():
            ...
            # Defensive mode is immutable — warn if config specifies otherwise
            if data.get("mode") != "defensive":
                _log.warning(
                    "policy.yaml specifies mode=%r; overriding to 'defensive' (non-negotiable)",
                    data.get("mode"),
                )
            data["mode"] = "defensive"
            return data
        return {
            ...
            "permissions": {"net": False, "fs_read": True, "fs_write": False},
            "limits": {"max_net_calls_per_run": 0, "max_output_kb_per_run": 2048},
        }
    ...
    def is_target_allowed(self, target: Target) -> tuple[bool, str]:
        """Returns (allowed, reason). Fails closed."""
        allow = self.policy.get("allow", {})

        if target.kind == "repo":
            allowed_repos: list[str] = allow.get("repos", [])
            if allowed_repos:
                if not any(
                    target.value.startswith(r) or r in target.value
                    for r in allowed_repos
                ):
                    if not target.proof:
                        return (
                            False,
                            f"Repo '{target.value}' not in policy allowlist and no proof provided",
                        )

        elif target.kind in ("folder", "config"):
            allowed_paths: list[str] = allow.get("local_paths", [])
            if allowed_paths:
                norm = str(Path(target.value).resolve())
                if not any(
                    norm.startswith(str(Path(p).resolve())) for p in allowed_paths
                ):
                    if not target.proof:
                        return (
                            False,
                            f"Path '{target.value}' not in policy local_paths allowlist",
                        )

        return True, "allowed"

    def net_allowed(self) -> bool:
        return bool(self.policy.get("permissions", {}).get("net", False))

    def max_net_calls(self) -> int:
        return int(
            self.policy.get("limits", {}).get("max_net_calls_per_run", 0)
        )
...
class BudgetPlanner:
    ...
    def plan(self, requested: list[str]) -> list[str]:
        ...
        for cap in requested:
            ...
            # Normalised cost: weighted average of fraction consumed
            fracs = [
        ...
        # Always guarantee at least one capability (partial results contract)
...
async def capability_inventory_repo(target: Target, run_id: str) -> EvidenceRecord:
    """Inventory all files + metadata in an owned local repo / folder."""
    t0 = time.monotonic()
    path = Path(target.value)

    if not path.exists():
        payload: dict[str, Any] = {
            "error": f"Path does not exist: {target.value}",
            "files": [],
        }
        ...
        payload = {
            ...
            "by_extension": by_ext,
            "files": files[:500],  # cap output to avoid blowing output_kb
        }
        summary = (
            f"Inventoried {len(files)} files "
            f"({total_size:,} bytes) in '{target.value}'"
        )

    elapsed_ms = int((time.monotonic() - t0) * 1000)
    return EvidenceRecord(
        run_id=run_id,
        capability="inventory_repo",
        provider="builtin.repo_inventory",
        summary=summary,
        payload=payload,
        cost=CostUsed(
            time_ms=elapsed_ms,
...
async def capability_scan_configs(target: Target, run_id: str) -> EvidenceRecord:
    """Scan owned config files for risky patterns (defensive analysis)."""
    t0 = time.monotonic()
    path = Path(target.value)
    findings: list[dict] = []
    scanned = 0
    ...
    if path.exists():
        for f in sorted(path.rglob("*")):
            if not f.is_file():
                continue
            if f.suffix not in _CONFIG_EXTS:
                continue
    ...
    summary = (
        f"Scanned {scanned} config files — "
        f"{len(findings)} risky pattern(s) found in '{target.value}'"
    )
    elapsed_ms = int((time.monotonic() - t0) * 1000)
    return EvidenceRecord(
        run_id=run_id,
        capability="scan_configs",
        provider="builtin.config_scan",
        summary=summary,
        payload=payload,
        cost=CostUsed(
            time_ms=elapsed_ms,
...
# Capability registry (extend here to add new defensive capabilities)
_CAPABILITY_MAP = {
...
async def run_intelligence(
    ...
    """
    تشغيل محرك الاستخبارات الدفاعي على مسار أو مستودع مملوك لك.

    Args:
        target_kind:  نوع الهدف — "folder" | "repo" | "config"
        target_value: مسار محلي مطلق أو رابط مستودع مملوك
        goal:         وصف هدف التحليل
        ram_mb:       الحد الأقصى للذاكرة (ميجابايت)
        time_ms:      الحد الأقصى للوقت الكلي (ملي ثانية)
        net_calls:    الحد الأقصى لطلبات الشبكة (0 = لا شبكة)
        output_kb:    الحد الأقصى لحجم الناتج (كيلوبايت)
        proof:        رمز إثبات الملكية (اختياري، يُستخدم لتجاوز القائمة البيضاء)
    """
    budget = Budget(
        ram_mb=ram_mb,
        time_ms=time_ms,
    ...
    # ── 1. Policy Gate ─────────────────────────────────────────────────────────
    allowed, reason = policy.is_target_allowed(target)
    if not allowed:
        return {"status": "denied", "reason": reason, "policy_hash": policy.policy_hash()}
    ...
    # ── 4. Execute planned capabilities ───────────────────────────────────────
    for cap in planned:
        handler = _CAPABILITY_MAP.get(cap)
        if not handler:
            continue
        cap_timeout = min(budget.time_ms / 1000, 120)  # hard cap 2 min per capability
        try:
            rec = await asyncio.wait_for(handler(target, run_id), timeout=cap_timeout)
            ledger.insert_evidence(rec)
            total_elapsed += rec.cost.time_ms
            results.append(
                {
                    "capability": cap,
                    "summary": rec.summary,
            ...
            run_status = "partial"
            results.append(
                {"capability": cap, "summary": "Timeout — partial results recorded", "cost": {}}
            )
        except (OSError, ValueError, KeyError, TypeError) as exc:
            run_status = "partial"
            results.append({"capability": cap, "summary": f"Error: {exc}", "cost": {}})
        except Exception as exc:  # noqa: BLE001 — unexpected errors still recorded, not swallowed
...
async def list_runs(limit: int = 10) -> list:
    """استعراض آخر تشغيلات محرك الاستخبارات من سجل الأدلة."""
    return ledger.list_runs(limit=limit)


@mcp.tool()
async def get_evidence(run_id: str, limit: int = 50) -> list:
    """استرجاع سجلات الأدلة الموقّعة لتشغيل معين من قاعدة البيانات."""
...
async def replay_run(run_id: str) -> dict:
    """
    إعادة استعراض تشغيل سابق كاملاً (Run + Evidence) للتدقيق أو المقارنة.
    يُتيح التحقق من ثبات النتائج (Replay Equivalence).
    """
...
async def check_policy() -> dict:
    """عرض سياسة الأمان الحالية وبصمتها التشفيرية."""
    return {
        "policy": policy.policy,
        "policy_hash": policy.policy_hash(),
        "mode": policy.policy.get("mode", "defensive"),
...
async def list_capabilities() -> dict:
    """
    عرض القدرات المتاحة في الكتالوج مع التكاليف المقدرة ومعاملات الكسب المعرفي.
    """
    caps: dict[str, dict] = {}
    ...
    # Also surface built-in capabilities not yet in catalog
...
async def budget_plan(
    ...
    مفيد لاستعراض ما سيتم تنفيذه قبل بدء التشغيل الفعلي.
    """
    budget = Budget(ram_mb=ram_mb, time_ms=time_ms, net_calls=net_calls, output_kb=output_kb)
    target = Target(kind=target_kind, value=target_value)
    ...
    estimated_cost: dict[str, dict] = {}
    for cap in planned:
        estimated_cost[cap] = catalog.estimated_cost(cap)

    return {
        "status": "plan_only",
        "budget": asdict(budget),
        "planned_capabilities": planned,
        "estimated_cost_per_capability": estimated_cost,
        "policy_hash": policy.policy_hash(),
    }
